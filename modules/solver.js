...
export const solve = (cube) => {
    ...
    const solution = solve_impl(cube, solved_cube, null, null, null, 6);
    if (solution !== null) {
        solution.reverse();
    }
    return solution;
...
const solve_impl = (cube, solved_cube, last_rot, last_axis1, last_axis2, max_depth) => {
    if (max_depth === 0) {
        return null;
    }
    ...
    for (const rotation of allowed_rotations) {
        ...
        // e.g. F followed by F', or F B followed by F
        if (base_rot === last_rot || (axis === last_axis1 && axis === last_axis2)) {
            continue;
        }
        // console.log('    '.repeat(max_depth) + rubik.rotations_to_string([rotation]));
        ...
        const possible_solution = solve_impl(
            cube, solved_cube, base_rot, last_axis2, axis, max_depth - 1);
